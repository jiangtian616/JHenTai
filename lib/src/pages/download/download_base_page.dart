...
class GroupedList<GROUP extends Widget, ELEMENT extends Widget> extends StatefulWidget {
  final Map<GROUP, List<ELEMENT>> datas;

  final ScrollController? scrollController;

  const GroupedList({super.key, required this.datas, this.scrollController});

  @override
  State<GroupedList<GROUP, ELEMENT>> createState() => _GroupedListState<GROUP, ELEMENT>();
}

class _GroupedListState<GROUP extends Widget, ELEMENT extends Widget> extends State<GroupedList<GROUP, ELEMENT>> {
  late ScrollController scrollController;
  bool useInnerController = false;
  ...
  @override
  void initState() {
    assert(widget.datas.keys.every((g) => g is StatelessWidget || g is StatefulWidget));
    assert(widget.datas.values.every((e) => e is StatelessWidget || e is StatefulWidget));
  ...
  @override
  void didUpdateWidget(covariant GroupedList<GROUP, ELEMENT> oldWidget) {
    super.didUpdateWidget(oldWidget);

    if (!MapEquality<GROUP, List<ELEMENT>>().equals(oldWidget.datas, widget.datas)) {
      _reRenderChildren(widget.datas);
    }
  }
  ...
  void _reRenderChildren(Map<GROUP, List<ELEMENT>> datas) {
    children.clear();

    datas.forEach((group, elements) {
      children.add(_buildGroup(group));
  ...
  Widget _buildElement(ELEMENT element) {
    return SliverToBoxAdapter(child: element);
  }
}
