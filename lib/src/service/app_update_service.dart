...
class AppUpdateService
  ...
  List<JHLifeCircleBean> get initDependencies => super.initDependencies
    ..addAll(updateHandlers.map((h) => h.initDependencies).expand((e) => e));
  ...
  Future<void> doInitBean() async {
    ...
    log.debug(
        'AppUpdateService fromVersion: $fromVersion, toVersion: $toVersion');
...
class FirstOpenHandler implements UpdateHandler {
  ...
  Future<bool> match(int? fromVersion, int toVersion) async {
    return fromVersion == null ||
        (await localConfigService.read(configKey: ConfigEnum.firstOpenInited) ==
            null);
  }
  ...
  Future<void> onReady() async {
    ...
    localConfigService.write(
        configKey: ConfigEnum.firstOpenInited, value: 'true');
  }
}
...
class StyleSettingMigrateHandler implements UpdateHandler {
  ...
  Future<void> onInit() async {
    ...
    if (styleSettingMap?['locale'] != null) {
      preferenceSetting
          .saveLanguage(localeCode2Locale(styleSettingMap!['locale']));
    }
    if (styleSettingMap?['enableTagZHTranslation'] != null) {
      preferenceSetting.saveEnableTagZHTranslation(
          styleSettingMap!['enableTagZHTranslation']);
    }
    if (styleSettingMap?['showR18GImageDirectly'] != null) {
      preferenceSetting
          .saveShowR18GImageDirectly(styleSettingMap!['showR18GImageDirectly']);
    }
    ...
    if (await downloadDir.exists()) {
      downloadDir.list().listen(
        (entity) async {
          ...
          File oldArchiveMetadataFile =
              File(join(entity.path, '.archive.metadata'));
...
class UpdateLocalGalleryPathHandler implements UpdateHandler {
  ...
  Future<void> onInit() async {
    ...
    downloadSetting
        .removeExtraGalleryScanPath(downloadSetting.defaultDownloadPath);
  }
...
class MigrateSearchConfigHandler implements UpdateHandler {
  ...
  Future<void> onReady() async {
    ...
    Map<String, dynamic>? map = storageService.read(
            '${ConfigEnum.searchConfig.key}: DesktopSearchPageTabLogic') ??
        storageService
            .read('${ConfigEnum.searchConfig.key}: SearchPageMobileV2Logic');
    if (map != null) {
...
class MigrateCookieHandler implements UpdateHandler {
  ...
  Future<void> onInit() async {
    ...
    File cookieFile = File(join(pathService.getVisibleDir().path, 'cookies',
        'ie0_ps1', 'exhentai.org'));
    if (!await cookieFile.exists()) {
      return;
    }

    String str = await cookieFile.readAsString();
    Map<String, Map<String, dynamic>> cookieStrs =
        json.decode(str).cast<String, Map<String, dynamic>>();
...
class MigrateLocalFilterTagsHandler implements UpdateHandler {
  @override
  List<JHLifeCircleBean> get initDependencies =>
      [storageService, localBlockRuleService];
  ...
  Future<void> onInit() async {
    ...
    if (map != null) {
      List<TagData> localTagSets = (map['localTagSets'] as List)
...
class MigrateGalleryHistoryHandler implements UpdateHandler {
  ...
  Future<bool> match(int? fromVersion, int toVersion) async {
    if (fromVersion == null) {
      await localConfigService.write(
          configKey: ConfigEnum.migrateGalleryHistory, value: 'true');
      return false;
    } else {
    ...
    for (int i = 0; i < pageCount; i++) {
      try {
        ...
        for (GalleryHistoryData history in oldHistories) {
          Future future =
              isolateService.jsonDecodeAsync(history.jsonBody).then((map) {
            return Gallery.fromJson(map);
          }).then((gallery) {
            return isolateService
                .jsonEncodeAsync(gallery2GalleryHistoryModel(gallery));
          }).then((galleryString) {
...
class MigrateStorageConfigHandler implements UpdateHandler {
  ...
  Future<bool> match(int? fromVersion, int toVersion) async {
    if (fromVersion == null) {
      await localConfigService.write(
          configKey: ConfigEnum.migrateStorageConfig, value: 'true');
      return false;
    } else {
  ...
  Future<void> onInit() async {
    ...
    Map? favoriteSettingMap =
        storageService.read(ConfigEnum.favoriteSetting.key);
    ...
    Map? advancedSettingMap =
        storageService.read(ConfigEnum.advancedSetting.key);
    ...
    Map? downloadSettingMap =
        storageService.read(ConfigEnum.downloadSetting.key);
    ...
    Map? performanceSettingMap =
        storageService.read(ConfigEnum.performanceSetting.key);
    ...
    Map? preferenceSettingMap =
        storageService.read(ConfigEnum.preferenceSetting.key);
    ...
    Map? securitySettingMap =
        storageService.read(ConfigEnum.securitySetting.key);
    ...
    Map? superResolutionSettingMap =
        storageService.read(ConfigEnum.superResolutionSetting.key);
    ...
    double? leftColumnWidthRatio =
        storageService.read(ConfigEnum.leftColumnWidthRatio.key);
    ...
    List<String>? cookies = storageService
        .read<List?>(ConfigEnum.ehCookie.key)
        ?.cast<String>()
        .toList();
    if (cookies != null) {
    ...
    Map<String, dynamic>? dashboardPageSearchConfigMap = storageService
        .read('${ConfigEnum.searchConfig.key}: DashboardPageLogic');
    if (dashboardPageSearchConfigMap != null) {
      SearchConfig searchConfig =
          SearchConfig.fromJson(dashboardPageSearchConfigMap);
      futures.add(localConfigService.write(
    ...
    if (searchPageSearchConfigMap != null) {
      SearchConfig searchConfig =
          SearchConfig.fromJson(searchPageSearchConfigMap);
      futures.add(
        localConfigService.write(
            configKey: ConfigEnum.searchConfig,
    ...
    Map<String, dynamic>? galleryPageSearchConfigMap = storageService
        .read('${ConfigEnum.searchConfig.key}: GallerysPageLogic');
    if (galleryPageSearchConfigMap != null) {
      SearchConfig searchConfig = SearchConfig.fromJson(galleryPageSearchConfigMap);
    ...
    Map<String, dynamic>? favoritePageSearchConfigMap = storageService
        .read('${ConfigEnum.searchConfig.key}: FavoritePageLogic');
    if (favoritePageSearchConfigMap != null) {
      SearchConfig searchConfig =
          SearchConfig.fromJson(favoritePageSearchConfigMap);
      futures.add(localConfigService.write(
          configKey: ConfigEnum.searchConfig,
    ...
    if (dismissVersion != null) {
      futures.add(localConfigService.write(
          configKey: ConfigEnum.dismissVersion, value: dismissVersion));
    }

    int? downloadPageBodyType =
        storageService.read(ConfigEnum.downloadPageBodyType.key);
    ...
    List? archiveDisplayGroups =
        storageService.read<List?>(ConfigEnum.displayArchiveGroups.key);
    ...
    List? galleryDisplayGroups =
        storageService.read<List?>(ConfigEnum.displayGalleryGroups.key);
    ...
    int? tagSearchOrderOptimizationServiceLoadingStateIndex = storageService
        .read(ConfigEnum.tagSearchOrderOptimizationServiceLoadingState.key);
    ...
    String? tagSearchOrderOptimizationServiceVersion = storageService
        .read(ConfigEnum.tagSearchOrderOptimizationServiceVersion.key);
    ...
    bool? showLocalBlockRulesGroup =
        storageService.read(ConfigEnum.displayBlockingRulesGroup.key);
  ...
  Future<void> onReady() async {
    ...
    if (keys != null) {
      ...
      keys
          .toList()
          .whereType<String>()
          .where((key) => key.startsWith(ConfigEnum.readIndexRecord.key))
          .forEach((key) {
      ...
      if (gid2ReadIndexMap.isNotEmpty) {
        List<LocalConfigCompanion> localConfigs =
            gid2ReadIndexMap.entries.map((entry) {
        ...
        for (List<LocalConfigCompanion> localConfigCompanions
            in localConfigs.partition(200)) {
