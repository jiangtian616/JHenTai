...
import 'gallery_download_service.dart' show DownloadStatus, GalleryDownloadProgress;

@Deprecated('Replaced by GalleryDownloadService')
class OldGalleryDownloadService extends GetxController {
  final String downloadGallerysId = 'downloadGallerysId';
  ...
  Map<int, OldGalleryDownloadSpeedComputer> gid2SpeedComputer = {};

  static Future<void> init() async {
    ensureDownloadDirExists();
    Get.put(OldGalleryDownloadService(), permanent: true);
  }
  ...
  AsyncTask<List<GalleryThumbnail>> _parseGalleryImageHrefTask(GalleryDownloadedData gallery, int thumbnailsPageNo) {
    return () {
      ...
      return EHRequest.requestDetailPage(
        ...
        thumbnailsPageIndex: thumbnailsPageNo,
  ...
  void _initGalleryDownloadInfoInMemory(GalleryDownloadedData gallery, {bool insertAtFirst = true}) {
    ...
    gid2DownloadProgress[gallery.gid] = GalleryDownloadProgress(
      ...
      hasDownloaded: List.generate(gallery.pageCount, (_) => false),
    );
    ...
    gid2SpeedComputer[gallery.gid] = OldGalleryDownloadSpeedComputer(
      gallery.pageCount,
  ...
  void _restoreDownloadInfoInMemory(GalleryDownloadedData gallery, List<GalleryImage?> images) {
    ...
    gid2DownloadProgress[gallery.gid] = GalleryDownloadProgress(
      curCount: hasDownloaded.where((e) => e == true).length,
      totalCount: gallery.pageCount,
...
class OldGalleryDownloadSpeedComputer extends SpeedComputer {
  late List<int> imageDownloadedBytes;
  late List<int> imageBytes;

  OldGalleryDownloadSpeedComputer(int pageCount, VoidCallback updateCallback)
